  ...
  <div class="main-content">
    ...
    <nav class="navbar navbar-top navbar-expand-md navbar-dark" id="navbar-main">
      <div class="container-fluid">
        <!-- Brand -->
        <a class="h4 mb-0 text-white text-uppercase d-none d-lg-inline-block" href="{{ route('user-appointments') }}">Cleaners</a>
        <!-- Form -->
          ...
          <div class="card shadow">
            <div class="card-header border-0">
              <div class="row align-items-center">
                <div class="col">
                  <h3 class="mb-0">Cleaners</h3>
                </div>

                <div class="col text-right">
                  <a href="#" data-toggle="modal" data-target="#AddCleaner" class="btn btn-sm btn-primary">Add Cleaner</a>
                </div>
            ...
            <div class="table-responsive">
              ...
              <table class="table align-items-center table-flush">
                ...
                @foreach($cleaners as $cleaner)
                  <tr>
                    <th scope="row">{{ $cleaner->sys_id }}</th>
                    <td>
                      <i class="far fa-eye text-success mr-3" data-toggle="modal" data-target="#ViewUserCleaner" title="View Cleaner" onclick="getCleanerProfile('{{ $cleaner->id }}');"></i>
                      <i class="fas fa-edit text-warning mr-3" title="Edit Cleaner" data-toggle="modal" data-target="#AddCleaner" onclick="getCleaner('{{ $cleaner->id }}','{{ $cleaner->first_name }}','{{ $cleaner->last_name }}','{{ $cleaner->id_number }}','{{ $cleaner->passport_number }}','{{ $cleaner->email }}','{{ $cleaner->phone }}','{{ $cleaner->phone1 }}','{{ $cleaner->gender }}','{{ $cleaner->photo }}','{{ $cleaner->country }}')"></i>
                      <i class="fas fa-trash text-danger mr-3" title="Delete Cleaner" data-toggle="modal" data-target="#deleteCleaner" onclick="deleteCleaner('{{ $cleaner->id }}');"></i>
                    </td>
                  </tr>
                @endforeach
